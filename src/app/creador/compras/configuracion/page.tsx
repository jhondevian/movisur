...
export const metadata: Metadata = {
  title: "Configuracion de compras | Movisur",
  description: "Metodos de pago del creador",
};

export default async function CreadorComprasConfiguracionPage() {
  const cookieStore = await cookies();
  const token = cookieStore.get(authCookieName)?.value;

  if (!token) redirect("/signin?next=/creador/compras/configuracion");
  ...
  try {
    ...
    redirect("/signin?next=/creador/compras/configuracion");
  }
