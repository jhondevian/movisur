...
export async function GET(request: NextRequest) {
  ...
  const offer = await prisma.creatorLicenseOffer.findUnique({
    where: {
      creatorId_planId: {
        creatorId: user.id,
        planId,
  ...
  const accounts =
    offer?.accounts.map((account) => ({
      account,
      assignedExpiresAt: getLicenseExpiration(
        account.assignedAt,
  ...
  await Promise.all(
    accounts
      ...
      .map(({ account, assignedExpiresAt }) =>
        prisma.creatorLicenseAccount.update({
          where: { id: account.id },
...
export async function POST(request: NextRequest) {
  ...
  if (!offer) {
    return NextResponse.json(
      { message: "El plan de licencia no existe." },
      { status: 404 }
    );
  }
